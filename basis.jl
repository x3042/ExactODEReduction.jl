...
function find_basis_1_beta(vectors)
    ...
    println("dim before eating = $(dim(alg))")
...
function owo()
    for (i, (mfn, mdim, msz, mdata)) in enumerate(load_MTX_if(from_dim=1, to_dim=50))

        @info "$i-th model : $mfn of dim : $mdim"

        As = map(matr -> from_COO(matr..., QQ), [ mdata ])

        V = find_basis(As, used_algorithm=find_basis_1_beta)
